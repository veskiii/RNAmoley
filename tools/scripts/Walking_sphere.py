...
class StructureProcessor:
    ...
    def _find_atoms_in_sphere(self, center_atom) -> List[Tuple[str, int]]:
        """Find all (chain_id, residue_id) pairs that have atoms within the sphere radius"""
        residue_pairs = set()
        ...
        for atom in self.structure.get_atoms():
            ...
            if distance <= self.radius:
                ...
                residue_pairs.add((chain_id, residue_id))

        return list(residue_pairs)

    def _write_sphere_file(self, center_atom, sphere_residues: List[Tuple[str, int]]) -> str:
        """Write sphere file containing all atoms from residues in sphere"""
        chain_id = center_atom.get_full_id()[2]
        residue_id = center_atom.get_full_id()[3][1]
        output_file = self.output_folder / f"{chain_id}_{residue_id}.pdb"
    ...
    def _write_sphere_from_pdb(self, output_file: Path, sphere_residues: List[Tuple[str, int]]) -> int:
            ...
            for line in original_lines:
                if line.startswith('ATOM') or line.startswith('HETATM'):
                    try:
                        ...
                        if (line_chain_id, line_residue_id) in sphere_residues:
                            sphere_file.write(line)
                            written_atoms += 1
    ...
    def _write_sphere_from_mmcif(self, output_file: Path, sphere_residues: List[Tuple[str, int]]) -> int:
            ...
            for model in self.structure:
                for chain in model:
                    for residue in chain:
                        ...
                        if (chain_id, res_num) in sphere_residues:
